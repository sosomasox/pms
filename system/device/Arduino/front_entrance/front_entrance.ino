...
void loop() {
  ...
  if(is_existing && !was_existed) {
    ...
    char data_json[] = "{'location':\"front_entrance\", 'target':\"subject\", 'subject':1}";
    ...
    if(count_of_time_s >= THRESHOLD_OF_COUNT_TIME) {
      ...
      char data_json[] = "{'location':\"front_entrance\", 'target':\"subject\", 'subject':0}";
      ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
  ...
  if(is_opening_the_door && !was_opened_the_door) {
    ...
    char data_json[] = "{'location':\"front_entrance\", 'target':\"door\", 'door':1}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
  ...
  // when someone close the door
    ...
    char data_json[] = "{'location':\"front_entrance\", 'target':\"door\", 'door':0}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));   

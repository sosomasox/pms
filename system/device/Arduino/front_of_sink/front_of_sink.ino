...
void loop() {
  ...
  if(is_existing && !was_existing) {
    ...
    char data_json[] = "{'location':\"front_of_sink\", 'target':\"subject\", 'subject':1}";
    ...
    if(count_of_time_s >= THRESHOLD_OF_COUNT_TIME) {
      ...
      char data_json[] = "{'location':\"front_of_sink\", 'target':\"subject\", 'subject':0}";
      ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
  ...
  if(is_turning_on_the_water_supply && !was_turned_on_the_water_supply) {
    ...
    char data_json[] = "{'location':\"front_of_sink\", 'target':\"water_supply\", 'water_supply':1}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
    ...
    char data_json[] = "{'location':\"front_of_sink\", 'target':\"water_supply\", 'water_supply':0}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));   

...
boolean flag_of_turned_on_the_tv = false;
...
inline boolean isTurningOnTheTV()
{
  int count = 0;
...
inline boolean wasTurnedOnTheTV()
{
  return flag_of_turned_on_the_tv;
}



inline boolean changeFlagOfTheTV()
{
  flag_of_turned_on_the_tv = !flag_of_turned_on_the_tv;
}
...
void loop() {
  ...
  if(is_existing && !was_existed) {
    ...
    char data_json[] = "{'location':\"living_room\", 'target':\"subject\", 'subject':1}";
    ...
    if(count_of_time_s >= THRESHOLD_OF_COUNT_TIME) {
      ...
      char data_json[] = "{'location':\"living_room\", 'target':\"subject\", 'subject':0}";
      ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
  ...
  boolean is_turning_on_the_tv = isTurningOnTheTV();
  boolean was_turned_on_the_tv = wasTurnedOnTheTV();

  if(is_turning_on_the_tv && !was_turned_on_the_tv) {
    changeFlagOfTheTV();
    
    char data_json[] = "{'location':\"living_room\", 'target':\"tv\", 'tv':1}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));
  ...
  else if(!is_turning_on_the_tv && was_turned_on_the_tv) {
    changeFlagOfTheTV();
    
    char data_json[] = "{'location':\"living_room\", 'target':\"tv\", 'tv':0}";
    ZBTxRequest zbTx = ZBTxRequest(addr64, data_json, strlen(data_json));   
